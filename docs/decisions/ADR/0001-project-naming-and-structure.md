...
- **状态**：Superseded by ADR-0031（§1 命名条款被 ADR-0031 取代；§2 目录结构条款仍有效）
- **生效日期**：2026-07-18
- **决策者**：首席架构工程师
...
2. **产品命名** 已分散落地：
   - [web/manifest.json](file:///d:/Projects/Active/math2/flutter_app/web/manifest.json): `"name": "formula_fix"`
   - [main.dart:26](file:///d:/Projects/Active/math2/flutter_app/lib/main.dart#L26): `title: 'FormulaFix'`
   - 文档存储文件名 `formula_fix_documents.json`（[document_service.dart:10](file:///d:/Projects/Active/math2/flutter_app/lib/core/services/document_service.dart#L10)）
   - 自动保存文件名 `formulafix_<timestamp>.md`（[file_service.dart:76](file:///d:/Projects/Active/math2/flutter_app/lib/core/services/file_service.dart#L76)）

3. **现有问题**：
   - `lib/providers/` 与 `lib/domain/providers/` 职责重叠，是 P0 阻塞问题（见 [CRITICAL_REVIEW.md §2.4](../../archive/audits/CRITICAL-REVIEW.md)）
...
- 删除 [lib/providers/editor_providers.dart](file:///d:/Projects/Active/math2/flutter_app/lib/providers/editor_providers.dart) 中重复的 `sharedPreferencesProvider` / `darkModeProvider`
...
- 项目根 `d:/Projects/Active/math2/` 持有 `docs/` / `AGENTS.md` / `.github/` 等工程治理文件
- `d:/Projects/Active/math2/flutter_app/` 是 Flutter 工程目录，持有 `pubspec.yaml` / `lib/` / `test/` / `web/`
...
- [CRITICAL_REVIEW.md](../../archive/audits/CRITICAL-REVIEW.md) §2.4
- [AGENTS.md §1](file:///d:/Projects/Active/math2/AGENTS.md) 项目架构原则
