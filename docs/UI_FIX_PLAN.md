...
|---|---|---|
| PR-A | 暗色阴影令牌体系 | P0-1 |
| PR-B | 主屏 SafeArea | P0-3（适配） |
| PR-C | 首页 golden + Golden 矩阵扩充 | P0-2 + P1-4 |
| PR-D | 响应式断点体系 | P1-1（适配） |
| PR-E | spacing/radius/字号令牌集中化 | P1-2 + P1-3 |
| PR-F | 组件 Widget + 死代码清理 + 即时颜色项（仅影响主题的黑/白） | P2-1/2 + P2-3收窄 |
| PR-G | 不抄设计稿设备装饰（首页 `SafeArea(top:false)` 只留系统状态栏） | P0-4 |
| PR-H | 颜色语义化长期治理（独立排期，逐文件小 PR） | P3 |
...
*本修复计划基于 `origin/main` = `5a481fc` 时刻的两轮只读审计，配套 `docs/UI_STATUS.md`。实施前请逐 PR 评审，避免单巨 PR。*
