...
| 3.3.2 | 字号缩放（双指缩放 + 按钮 + 重置） | P1（v1.3 降级,R3确认） | 🔴零代码/PR#4待开发 | ✅ E2E：打开文档 → 点击放大 → 文本字号变化 → 点击重置 → 恢复默认字号；移动端双指缩放手势验证 |
| 3.3.3 | 焦点模式（隐藏 chrome,双击退出） | P1 | 🔴零代码/PR#4待开发 | ✅ E2E：进入编辑 → 双击进入焦点模式 → AppBar/Toolbar/StatusBar 隐藏 → 双击退出 → Chrome 恢复 |
...
- **v1.6 状态盘点（2026-07-25 代码核对）**：9 个子任务中 7 个开发代码已实现并接线（flutter analyze 0 error）：3.3.1/3.3.4/3.3.5（PR #1 chrome 接线,`kEnableNewEditor=true` 已激活）+ 3.3.7/3.3.10（PR #2 工具栏+模板 dispatch 接线）+ 3.3.6/3.3.8（PR #3 已合 `7f53eec`）。**仅 3.3.2 字号缩放 + 3.3.3 焦点模式（PR #4）为零代码,是 Phase 3.3 唯一剩余开发任务。** 所有 `✅开发完` 项仍须按 §12.2 补 E2E（去 `skip:true`）后方可标记整体通过；契约表 `⏳` 已校正为上述状态。
