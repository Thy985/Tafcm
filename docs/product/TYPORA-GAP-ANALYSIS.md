# Phase 3.10 — FormulaFix vs Typora 类产品差距分析
...
FormulaFix 在**「数学富文本编辑器」这一差异化主轴**上已具备 Typora 级骨架（真 WYSIWYG、数学公式、Mermaid、表格、本地 .md 单一真相源、自动保存）；但在 **「写作流舒适性功能」**（打字机/焦点暗化/快捷键/选区气泡/图片拖放）与 **「格式与导出广度」**（自定义 CSS、源码视图、多导出格式、脚注交叉引用）上明显落后——属于**「专业数学编辑器雏形」而非「完整 Markdown 写作器」**。

> ⚠️ 定位差异：Typora 是**桌面端通用写作器**；FormulaFix 是**移动端优先的数学写作器**。因此部分差距是**产品定位差异（合理）**，部分是**真实功能缺失（应补）**。本分析区分二者。

---

## 1. 能力矩阵（25 项，代码实证）

| # | 能力 | Typora | FormulaFix | 代码证据 | 判定 |
|---|------|-------|-----------|---------|------|
...
> 这些应归入 Baseline v1 的 **Core Stable**——已是FormulaFix 的实做到位功能。
...
### 3.5 平台广度（定位差异，非缺陷）
- Typora 桌面三端 + beta；FormulaFix 当前 Android 单端，桌面/Web 在 Phase 4。
...
*本文件为 FormulaFix vs Typora 差距分析（v1.0，代码实证）。*
