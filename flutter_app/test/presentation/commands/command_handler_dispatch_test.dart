/// R4 自省测试：验证所�?[EditorCommand] 子类都有对应�?`_handle*` 分支�?///
/// 落地 PR 评审 R4（Phase 2.9 PR review�? ADR-0009 §3.2�?///
/// **背景**：`CommandHandler._dispatch` 使用 if-else 链分派（�?[EditorCommand]
/// �?sealed，编译器不强制穷举）。若新增 [EditorCommand] 子类时忘记在
/// `_dispatch` 添加分支，会静默返回 false（无编译错误、无运行时异常）�?///
/// **守门策略**：本测试为每�?[EditorCommand] 子类构造一�?*应成�?*的场景，
/// 调用 `handler.handle(command)` 并验证：
/// 1. 返回 true（dispatch 命中对应分支�?/// 2. Editor 状态发生预期变化（实际执行�?op，而非 dispatch 静默失败�?///
/// **Phase 3.0 升级路径**（详�?command_handler.dart doc comment）：
/// �?[EditorCommand] 转为 sealed class，改�?switch 表达式强制穷举，
/// 本测试可简化为编译期保证（仅需检�?switch 是否 exhaustive）�?library;
...
void main() {
  group('R4 自省：所�?EditorCommand 子类都有对应 _handle* 分支', () {
    ...
    test('SplitBlockCommand �?dispatch', () {
      ...
      expect(success, isTrue,
          reason: 'SplitBlockCommand 应被 dispatch（否�?_dispatch 漏了分支�?);
      expect(editor.blockCount, equals(2),
          reason: 'dispatch 成功后块数应�?1 变为 2');
    });

    test('MergeWithPreviousCommand �?dispatch', () {
      ...
      expect(editor.blockCount, equals(1),
          reason: 'dispatch 成功后块数应�?2 变为 1（id2 合并�?id1�?);
    });

    test('InsertBlockAfterCommand �?dispatch', () {
    ...
    test('DeleteBlockCommand �?dispatch', () {
      ...
      expect(editor.blockCount, equals(1),
          reason: 'dispatch 成功后块数应�?2 变为 1');
    });

    test('MoveBlockUpCommand �?dispatch', () {
      editor.addParagraph('hello');
      final id2 = editor.addParagraph('world');
      ...
      // 块数不变，但顺序变了：world 应在�?      expect(editor.blockCount, equals(2));
      expect(editor.allSources.first, equals('world'),
          reason: 'id2 上移后应在第一�?);
    });

    test('MoveBlockDownCommand �?dispatch', () {
      final id1 = editor.addParagraph('hello');
      editor.addParagraph('world');
    ...
    test('UpdateBlockSourceCommand �?dispatch', () {
      final id = editor.addParagraph('hello');
      ...
      expect(editor.sourceOf(id), equals('updated'),
          reason: 'dispatch 成功�?source 应被替换');
    });

    test('TransformBlockCommand �?dispatch', () {
      ...
      expect(editor.getBlock(id), isA<HeadingElement>(),
          reason: 'dispatch 成功�?ParagraphElement 应被 transform �?HeadingElement');
    });
  });

  // Phase 3.3 PR #2A 新增�?4 个测试组（InsertTextCommand / WrapSelectionCommand /
  // InsertTemplateCommand / Undo-Redo 集成）已拆分�?  // test/presentation/commands/command_handler_pr2a_test.dart（AGENTS.md §1.2）�?
  group('R4 守卫测试：_handle* 方法的边界守�?, () {
    late InMemoryDocumentEditor editor;
    late EditorHistory history;
    late CommandHandler handler;
    ...
    test('MergeWithPreviousCommand 首块守卫：返�?false 且块数不�?, () {
      ...
      expect(success, isFalse,
          reason: '首块无法 merge，应�?_handleMerge 守卫拦截');
      expect(editor.blockCount, equals(1),
          reason: '守卫触发时块数不�?);
    });

    test('MoveBlockUpCommand 首块守卫：返�?false 且顺序不�?, () {
      ...
      expect(success, isFalse,
          reason: '首块无法上移，应�?_handleMoveUp 守卫拦截');
      expect(editor.allSources, equals(['first', 'second']),
          reason: '守卫触发时顺序不�?);
    });

    test('MoveBlockDownCommand 末块守卫：返�?false 且顺序不�?, () {
      ...
      expect(success, isFalse,
          reason: '末块无法下移，应�?_handleMoveDown 守卫拦截');
      expect(editor.allSources, equals(['first', 'last']),
          reason: '守卫触发时顺序不�?);
    });

    test('DeleteBlockCommand 单块守卫：返�?false 且块数不�?, () {
      // BlockOperations.delete 的守卫：blockCount <= 1 时返�?false
      ...
      expect(success, isFalse,
          reason: '单块无法删除，应�?BlockOperations.delete 守卫拦截');
      expect(editor.blockCount, equals(1),
          reason: '守卫触发时块数不�?);
    });
  });
