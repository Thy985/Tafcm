/// Phase 3.3 PR #2A�? 个新 Command 子类�?dispatch + 行为 + Undo/Redo 集成测试�?///
/// 落地 Task Contract v2.1 §4.4 + §5.4.1（selection offset 必须验证）�?///
/// �?`command_handler_dispatch_test.dart` 拆分（AGENTS.md §1.2 文件 �?400 行）�?/// 本文件专�?PR #2A 新增�?[InsertTextCommand] / [WrapSelectionCommand] /
/// [InsertTemplateCommand]，原文件保留 R4 自省 + R4 守卫测试�?library;
...
void main() {
  group('Phase 3.3 PR #2A: InsertTextCommand dispatch + 行为', () {
    ...
    test('InsertTextCommand 有选区时替换选区内容', () {
      ...
      expect(success, isTrue);
      // selection.baseOffset=6,�?6 处插�?'dart',�?[6..11] 不变但被推后
      // 实际：hello [插入 dart] world �?hello dartworld
      // 注意：InsertTextCommand 仅插�?不删除选区（删除选区�?WrapSelection 的职责）
    ...
    test('InsertTextCommand 越界守卫：baseOffset > source.length 返回 false', () {
      ...
      expect(editor.sourceOf(id), equals('hello'),
          reason: '守卫触发�?source 不变');
    });

    test('InsertTextCommand 找不�?blockId 返回 false', () {
  ...
  group('Phase 3.3 PR #2A: WrapSelectionCommand dispatch + 行为', () {
    ...
    test('WrapSelectionCommand �?dispatch�?* 包裹�?, () {
      final id = editor.addParagraph('hello world');
      // 选中 "hello" [0..5]
    ...
    test('WrapSelectionCommand 反向选区（baseOffset > extentOffset）正确包�?,
        () {
      final id = editor.addParagraph('hello world');
  ...
  group('Phase 3.3 PR #2A: InsertTemplateCommand dispatch + 行为', () {
    ...
    test('InsertTemplateCommand �?dispatch（insert 模式�?, () {
      final id = editor.addParagraph('hello');
    ...
    test('InsertTemplateCommand �?dispatch（newBlock 模式�?, () {
    ...
    test('InsertTemplateCommand insert 模式复用 InsertText 越界守卫', () {
      ...
      expect(success, isFalse,
          reason: 'insert 模式应继�?InsertText 的越界守�?);
    });

    test('InsertTemplateCommand 找不�?blockId 返回 false', () {
      const invalidId = BlockId('999');
      ...
      final success = handler.handle(cmd);
      expect(success, isFalse,
          reason: '找不�?blockId 应返�?false（newBlock 依赖 blockId 存在�?);
    });
  });

  group('Phase 3.3 PR #2A: �?Command �?Undo/Redo 集成', () {
    late InMemoryDocumentEditor editor;
    late EditorHistory history;
    late CommandHandler handler;
    ...
    test('InsertTextCommand 执行�?Undo 还原 source', () {
      final id = editor.addParagraph('hello');
      // 执行 InsertText
      ...
      expect(history.canUndo, isTrue,
          reason: 'InsertText 执行后应�?Undo');

      // Undo：revert 所�?op（与 transaction_history_integration_test 一致）
      final undone = history.undo(history.lastOrNull!);
      for (final op in undone!.ops.reversed) {
        op.revert(editor);
      }
      expect(editor.sourceOf(id), equals('hello'),
          reason: 'Undo �?source 应还�?);
    });

    test('WrapSelectionCommand 执行�?Undo 还原 source', () {
      final id = editor.addParagraph('hello');
      ...
      expect(editor.sourceOf(id), equals('hello'),
          reason: 'Undo �?WrapSelection 应被还原');
    });

    test('InsertTemplateCommand (newBlock) 执行�?Undo 还原块数', () {
      ...
      expect(editor.blockCount, equals(1),
          reason: 'Undo �?newBlock 应被还原（块数回�?1�?);
    });
  });
}
