...
/// **覆盖范围**�?/// - 4 种配对符触发检测（`(` / `[` / `{` / `` ` ``�?/// - 非配对符不触发（`*` / `$` / `#` / 普通字符）
/// - 有选区时不触发（wrapSelection �?Phase 3.4+�?/// - 删除 / 粘贴多字符不触发
/// - insertOffset + cursorOffset 正确�?library;
...
void main() {
  ...
  /// 构�?oldValue（光标在末尾的空文本或指定文本）�?  TextEditingValue oldValueWith(String text, {int? cursor}) =>
      TextEditingValue(
        text: text,
        selection: TextSelection.collapsed(offset: cursor ?? text.length),
      );

  /// 模拟用户在光标位置输�?1 个字符�?  TextEditingValue typeChar(String text, String ch, {int? cursor}) {
  ...
  group('AutoPairRules.detect �?4 种配对符', () {
    test('输入 ( �?返回 PairInsertCommand(suffix=)), cursorOffset=-1)', () {
      ...
      expect(cmd.insertOffset, 6); // 光标�?'(' 之后
      expect(cmd.cursorOffset, -1); // 光标前移 1（在 '()' 中间�?      expect(cmd.mode, PairInsertMode.appendAfterCursor);
  ...
  group('AutoPairRules.detect �?不触发场�?, () {
    test('有选区�?�?返回 null（wrapSelection �?Phase 3.4+�?, () {
  ...
  group('AutoPairRules.detect �?insertOffset + cursorOffset 正确�?, () {
    test('在文本中间输�?( �?insertOffset = 光标位置', () {
      ...
      expect(cmd, isNotNull);
      expect(cmd!.insertOffset, 3); // 光标�?'(' 之后 = position 3
      expect(cmd.cursorOffset, -1);
    });

    test('在文本开头输�?[ �?insertOffset = 1', () {
