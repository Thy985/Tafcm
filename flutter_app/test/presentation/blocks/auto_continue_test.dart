...
/// **覆盖范围**�?/// - 5 种前缀触发续行（`- ` / `* ` / `1. ` / `> ` / `- [ ] `�?/// - 5 种前缀触发退出（空内�?+ 回车�?/// - v1.1 P1-5：checkbox 优先�?`- `（不被抢先匹配）
/// - 有序列表编号递增（`1.` �?`2.` / `10.` �?`11.`�?/// - 不触发场景（普通文�?/ �?/ 不以 `\n` 结尾 / 标题�?/// - 多行场景（倒数第二行前缀检测）
library;
...
void main() {
  ...
  group('AutoContinueRules.detect �?v1.1 P1-5 checkbox 优先�?, () {
    test('- [ ] task 不被 - 抢先匹配（续�?"- [ ] " 而非 "- ")', () {
      final cmd = AutoContinueRules.detect(
        newValue: pressEnter('- [ ] task'),
        blockId: blockId,
      );
    ...
    test('- [x] task（已完成 checkbox）→ 匹配 "- " 而非 checkbox', () {
      // 已完�?checkbox 不在优先�?1 �?regex 中（仅匹�?"- [ ] "�?      // 应降级到优先�?4�?- "），续行 "- "
      final cmd = AutoContinueRules.detect(
  ...
  group('AutoContinueRules.detect �?不触发场�?, () {
    test('普通文�?+ 回车 �?返回 null', () {
      final cmd = AutoContinueRules.detect(
  ...
  group('AutoContinueRules.detect �?Command 字段验证（v1.1 P0-3�?, () {
    test('InsertNewLineWithPrefixCommand �?currentSource 字段（编译时保证�?, () {
      // 此测试仅验证 Command 字段可访问，currentSource 字段不存在由编译器保�?      final cmd = AutoContinueRules.detect(
        newValue: pressEnter('- item'),
        blockId: blockId,
      );
      ...
      expect(cmd.displayName, '自动续列�?);
      // currentSource 字段不存在：访问会编译错误，无需运行时测�?    });

    test('退�?Command �?origin 也为 ime', () {
      final cmd = AutoContinueRules.detect(
        newValue: pressEnter('- '),
        blockId: blockId,
      );
