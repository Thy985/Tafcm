/// ADR-0020 D3 spike：CommandHandler 失败原子性 + typing-session 粒度验证。
///
/// 验证：
/// - 成功命令 → 编辑器变更 + 恰好 1 个历史条目（typing session = 1 Transaction）
/// - 失败命令 → 编辑器状态精确恢复（无部分变异泄漏）
/// - revertBuilder helper → 多 op 事务逆序 revert 精确恢复（D3 原子性核心）
/// - revertBuilder 后可立即开新事务（不阻塞）
///
/// 落地 ADR-0020 D3（E 期 spike）。
library;
...
void main() {
  group('ADR-0020 D3 spike: CommandHandler 原子性', () {
    ...
    test('成功命令：编辑器变更 + 恰好 1 个历史条目（typing session 粒度）', () {
      ...
      // 一个命令 = 一个 Transaction = 一个 undo 步（D3 粒度：typing session 合并）
    ...
    test('失败命令：编辑器状态精确恢复（无部分变异泄漏）', () {
      ...
      // 第一块无法合并 → 失败；_handleMerge 在 ops.merge 前返回 false（不变异），
      // 失败路径 revertBuilder 亦不应留下任何残留。
      final ok = handler.handle(MergeWithPreviousCommand(blockId: id));
      expect(ok, isFalse);
      expect(editor.allIds.map((i) => editor.sourceOf(i)).toList(),
          equals(initialSources),
          reason: '失败命令后编辑器应完全未变');
