...
> **版本**：v1.0（2026-07-29）
> **作者**：AI 协作开发者
> **关联**：`docs/spec/editor-interaction.md`（本 ADR 的规范载体）· ADR-0018 · ADR-0012 · 真机问题报告 `phase3.5-realdevice-issues.md`
...
2. **采用 `docs/spec/editor-interaction.md` 为唯一行为真相源**，其中 §3 Enter 矩阵 / §4 行为 / §5 Live 同步规则 / §9 验收标准为本 ADR 的强制约束。
...
6. **集中裁决，禁止 per-block 行为**：Intent → Command 的映射集中在 `BlockBehaviorResolver`（单一 `switch(blockType)`），UI 层统一经 `EditorIntentDispatcher.dispatch(Intent)`。禁止在 Block 子类写 `ParagraphBlock.onEnter()` 之类 per-block 行为方法——未来新增 Table / MathBlock / Callout / Image Caption 只需在 resolver 增加分支，无需改动任何 Block 组件。
...
- **正面**：输入行为可预测、可测试；新增 block 类型只需补矩阵分支；工具栏/快捷键统一经 dispatcher，消除 ad-hoc 发散。
...
- **守门**：
  ...
  - 架构测试候选（TC-ARCH-EDITOR-1）：UI 层禁止直接 `coordinator.handle(SplitBlockCommand(...))`，必须经 `dispatch(Intent)`。
...
- [ ] 关联 PR：编辑器内核重构（替代 `fix/editor-kernel`）
