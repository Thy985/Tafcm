  ...
  Future<void> _load() async {
    ...
    try {
      ...
      for (final doc in docs) {
        if (!mounted) break;
        final preview = await repo.getDocumentPreview(doc.id);
        if (mounted) setState(() => _previews[doc.id] = preview);
      }
    } catch (_) {
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  if (_loading)
                    ...
                    _DocList(docs: recent, previews: _previews, onTap: _openDoc),
                    // 更早
                    ...
                    _DocList(docs: earlier, previews: _previews, onTap: _openDoc),
                  ],
...
class _DocList extends StatelessWidget {
  ...
  const _DocList({required this.docs, required this.previews, required this.onTap});
